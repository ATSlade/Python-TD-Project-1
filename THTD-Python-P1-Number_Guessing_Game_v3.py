...
# Greet the player and explain the game
    ...
    guess_count = 0
    user_guess = 3759007538
    # On a loop, prompt the player to 'guess' the number
    while correct == False:
        # Tracking the user's guesses so that the game does not count it against them
        while user_guess not in guess_board:
            user_guess = input("\nWhat's your guess?: ")  
            guess_board.append(user_guess)
            print(guess_board) # T E S T I N G    

            guess_count += 1
            print("\nThis is guess #{}...".format(guess_count)) # T E S T I N G
            ...
            # If guess = rando, display "Got it"
                ...
                correct = True
        print("You've already guessed that number, so won't count it against you. Please try again.\n")
        guess_count -= 1    
